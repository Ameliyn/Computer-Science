...
def generate_data(m, coefficients):
    ...
    """
    x = np.array(np.arange(-5.0, 5.00001, 10.0/(m-1))).reshape(m,1)
...
def plot_curve(degree, model):
    ...
    """
    x = np.array(np.arange(-5.0, 5.00001, 10.0 / (100 - 1)))
    y = [model.intercept_ for i in range(100)]
    coefs = model.coef_
    print(coefs[0])
...
def mse(X, y, degree, model):
    ...
    """
    # TODO You have to write this. Start by removing the line below.
    pass
