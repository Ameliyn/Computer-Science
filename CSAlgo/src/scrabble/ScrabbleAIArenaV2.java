...
public class ScrabbleAIArenaV2 {
    ...
    private void Reset(int firstPlayer){
        //CHOOSE ON THIS LINE
        Contestants = new ScrabbleAI[]{new BrutusV2_0(), new Incrementalist(), new Incrementalist()};

        board = new Board(3);
        if(firstPlayer % 3 == 0){ai1 = Contestants[0]; ai2 = Contestants[1]; ai3 = Contestants[2];}
        else if(firstPlayer % 3 == 1){ai1 = Contestants[2]; ai2 = Contestants[0]; ai3 = Contestants[1];}
        else {ai1 = Contestants[1]; ai2 = Contestants[2]; ai3 = Contestants[0];}
        ai1Name = ai1.toString().substring(9).split("@")[0];
        ai2Name = ai2.toString().substring(9).split("@")[0];
        ai3Name = ai3.toString().substring(9).split("@")[0];
        
        ai1.setGateKeeper(new GateKeeper(board, 0));
        ai2.setGateKeeper(new GateKeeper(board, 1));
        ai3.setGateKeeper(new GateKeeper(board, 2));
        mode = Mode.AI1_PLAYING;
    }

    public static void main(String[] args) throws IllegalMoveException {
        new ScrabbleAIArenaV2().run();
    }
            ...
            while (mode != Mode.GAME_OVER) {
                if (mode == Mode.AI1_PLAYING) {
                    ...
                    if (board.gameIsOver()) {
                        ...
                        mode = Mode.AI3_PLAYING;
                    }
                    draw();
                }
                else{
                    draw();
                    ScrabbleMove move = ai3.chooseMove();
                    // This fixes a security hole where the AI player returns an instance of a new class implementing
                    // ScrabbleMove, which then manipulates the Board.
                    if (!(move instanceof PlayWord || move instanceof ExchangeTiles)){
                        throw new IllegalMoveException("Bogus ScrabbleMove implementation detected!");
                    }
                    Location[] place = move.play(board, 2);
                    if (place != null) {
                        boardCursor = place[0];
                        boardCursorDirection = place[1];
                    }
            ...
            do{
                ...
                if(c == 'q')
                {
                    ...
                    if((numRuns-1) % 3 == 0){
                        ...
                        StdOut.println(ai3Name + ": " + wins3 + " (Total: " + score3 +
                                ", Average: " + ((float)score3 / numRuns) +  ")");
                    }
                    else if((numRuns-1) % 3 == 1){
                        StdOut.println(ai1Name + ": " + wins3 + " (Total: " + score3 +
                                ", Average: " + ((float)score3 / numRuns) + ")");
                        StdOut.println(ai2Name + ": " + wins1 + " (Total: " + score1 +
                                ", Average: " + ((float)score1 / numRuns) +  ")");
                        StdOut.println(ai3Name + ": " + wins2 + " (Total: " + score2 +
                                ", Average: " + ((float)score2 / numRuns) +  ")");
                    }
                    else{
                        StdOut.println(ai1Name + ": " + wins3 + " (Total: " + score3 +
...
//                            " games (" + (Math.round((Math.abs(score1-score2) * 100.0) / (numRuns)) / 100) + " per game)");
    ...
    private void draw() {
        ...
        StdDraw.text(7, 15, ai1Name + " vs " + ai2Name + " vs " + ai3Name + " Game " + (numRuns+1));
        ...
        hand = board.getHand(2);
        for (int i = 0; i < hand.size(); i++) {
            drawSquare(16 + i, 8, hand.get(i), mode == Mode.HAND && handCursor == i,
                    mode == Mode.HAND && tilesToDiscard[i], false);
        }
        ...
        StdDraw.text(19, 7, ai3Name + ": " + board.getScore(2));
        ...
        if (mode == Mode.BOARD) {
            ...
            if(numRuns % 3 == 0) {
                ...
                score3 += board.getScore(2);
            }
            else if(numRuns % 3 == 1){
                score1 += board.getScore(2);
                score2 += board.getScore(0);
                score3 += board.getScore(1);
            }
            else {
                score1 += board.getScore(1);
                score2 += board.getScore(2);
                score3 += board.getScore(0);
            }

            if(board.getScore(0) > board.getScore(1) && board.getScore(0) > board.getScore(2)){
                if(numRuns % 3 == 0) {
                    wins1++;
                }
                else if(numRuns % 3 == 1){
            ...
            else if(board.getScore(1) > board.getScore(0) && board.getScore(1) > board.getScore(2)){
                if(numRuns % 3 == 0) {
                    wins2++;
                }
                else if(numRuns % 3 == 1){
            ...
            else if(board.getScore(2) > board.getScore(0) && board.getScore(2) > board.getScore(1)){
                if(numRuns % 3 == 0) {
                    wins3++;
                }
                else if(numRuns % 3 == 1){
                ...
                StdDraw.text(19, 6, ai3Name + " Wins!");
            }
            else{
            ...
            if(numRuns % 3 == 0){
                StdDraw.text(19, 4, ai1Name + ": " + wins1 + " (" + score1 + ")");
                StdDraw.text(19, 3, ai2Name + ": " + wins2 + " (" + score2 + ")");
                StdDraw.text(19, 2, ai3Name + ": " + wins3 + " (" + score3 + ")");
            }
            else if(numRuns % 3 == 1){
                StdDraw.text(19, 4, ai1Name + ": " + wins3 + " (" + score3 + ")");
                StdDraw.text(19, 3, ai2Name + ": " + wins1 + " (" + score1 + ")");
                StdDraw.text(19, 2, ai3Name + ": " + wins2 + " (" + score2 + ")");
            }
            else{
                StdDraw.text(19, 4, ai1Name + ": " + wins2 + " (" + score2 + ")");
                StdDraw.text(19, 3, ai2Name + ": " + wins3 + " (" + score3 + ")");
                StdDraw.text(19, 2, ai3Name + ": " + wins1 + " (" + score1 + ")");
            }
            StdDraw.text(19, 1, "Score Differential: " + Math.abs(score1 - score2) + " over " + (numRuns+1) + " games");
            StdDraw.text(19, 0, "Press enter to play again.");
            ...
            if(numRuns %3 == 0){
                ...
                StdOut.println(ai3Name + ": " + wins3 + " (" + board.getScore(2) + ")");
            }
            else if(numRuns % 3 == 1){
                StdOut.println(ai1Name + ": " + wins3 + " (" + board.getScore(2) + ")");
                StdOut.println(ai2Name + ": " + wins1 + " (" + board.getScore(0) + ")");
                StdOut.println(ai3Name + ": " + wins2 + " (" + board.getScore(1) + ")");
            }
            else{
                StdOut.println(ai2Name + ": " + wins2 + " (" + board.getScore(1) + ")");
                StdOut.println(ai1Name + ": " + wins3 + " (" + board.getScore(2) + ")");
        ...
        }else if(mode == Mode.AI3_PLAYING){
            StdDraw.setPenColor(Color.WHITE);
            StdDraw.setFont(INTERFACE_FONT);
            StdDraw.text(19, 6, ai3Name + " thinking...");
        }
        StdDraw.show();
    }
