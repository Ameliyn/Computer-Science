...
// creates a torus with hole ratio
int create_torus(double m[4][4], double ir, double ig, double ib, char * texture, double centerRat){
  tanhalfangle = tan(halfangle);
  double irgb[3] = {ir,ig,ib};
  double argb[3];
  double p[3], r[3], q[3];
  ...
  for(double u = ulo; u <= uhi; u +=inc) {
    for(double v = vlo; v <= vhi; v +=inc) {
      ...
      q[0] = (c + a*cos(v)) * cos(u+inc);
      q[1] = (c + a*cos(v)) * sin(u+inc);
      q[2] = a * sin(v);
      
      r[0] = (c + a*cos(v+inc)) * cos(u);
      r[1] = (c + a*cos(v+inc)) * sin(u);
      r[2] = a * sin(v+inc);
...
int testTorus(double eye[4][4]){
  ...
  if(debug) printf("Creating torus\n");
  create_torus(V,1,0.0,1.0,"null",0.9);
